...
module.exports = class updateLocationAction extends Action {
    constructor() {
        super();
    ...
    async executeFunction(data) {
        try {
            const result = await api.location.updateLocation(data.params.locationName, data.params.pathToUpdate, data.params.valueToUpdate);
            return { data: result };
        } catch (error) {
            return { err: error };
        }
