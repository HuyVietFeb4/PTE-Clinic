...
module.exports = class changePasswordClientAction extends Action {
    constructor() {
        super();
        this.name = 'changePasswordClient';
        this.description = 'Change password client action';
    ...
    async executeFunction(data) {
        try {
            const result = await api.user.changePasswordClient(data.params.clientEmail, data.params.oldPassword, data.params.newPassword);
            return { data: result };
        } catch (error) {
            return { err: error };
        }
