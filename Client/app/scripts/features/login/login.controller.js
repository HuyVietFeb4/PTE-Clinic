angular.module('login').controller('clientLoginController', function (apiLogin) {
    ...
    this.clientLogin = function() {
        ...
        apiLogin.clientLoginAction(params).then(function(response) {
            authenticationAlert('Client login', response.data.success, response.data.message, this.showAlert);
            ...
            authenticationAlert('Client login admin login', false, errorMsg, this.showAlert);
        });
    }
});
